...
# Launcher for devin-desktop using the system Electron runtime.
# Based on Arch Linux's code.sh launcher pattern: cli.js runs in Node mode
# to handle CLI args, then respawns Electron with --app= to point system
# electron at the correct application directory.

_name=@@ELECTRON@@
_app=/opt/devin-desktop/resources/app
...
_flags_file="${XDG_CONFIG_HOME:-$HOME/.config}/devin-desktop-flags.conf"
_codeflags=()
...
ELECTRON_RUN_AS_NODE=1 exec "/usr/lib/${_name}/electron" "${_cli}" "${_electronflags[@]}" --app="${_app}" "${_codeflags[@]}" "$@"
