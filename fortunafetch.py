...
def get_info():
    ...
    # Если батареи нет - не добавляем ключ в словарь
...
def print_combined_view(info, width):
    ...
    # Обёртываем значения с переносом по ширине value_width
    wrapped_items = []
    for k, v in items:
    ...
    # Максимальное число строк — либо логотип, либо суммарное число строк по инфе
...
def ftk_ad():
    print(
        ...
        "│           Ad section          │\n",
        "└───────────────────────────────┘\n",
        "ТОЛЬКО СЕЙЧАС!!! Тариф дачный, всего за 1500 руб/мес\n",
        "вы получите 10 мбит/с и 25 мбит/с по ночам, все ночные приключения на скорости 2.5x\n",
        "\n",
        "ФТК Бет - зарегестрируйтесь на ФТК Бет\n",
        "и только сейчас получите 100 фрибетов и 250 подачному тарифу\n",
        "\n",
        "ФТК VPN - 1 гб/с всего за 150 руб.\n", 
        "Первые 100 пользователей получат БЕСКОНЕЧНУЮ подписку\n",
    )
