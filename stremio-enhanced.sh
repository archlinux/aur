...
if [[ "${EUID}" -ne 0 ]] || [[ "${ELECTRON_RUN_AS_NODE}" ]]; then
    exec electron@electronversion@ --app-name="@appname@" "${_RUNNAME}" "${_OPTIONS}" "${flags[@]}" "$@" || exit $?
else
    exec electron@electronversion@ --app-name="@appname@" "${_RUNNAME}" "${_OPTIONS}" --no-sandbox "${flags[@]}" "$@" || exit $?
fi