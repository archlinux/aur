...
# plutosvg comes from Arch's official repository. discord-rpc (AUR) is loaded
# with dlopen() at runtime, so it needs no CMake package or import target here;
# its header is found on the default system include path.
find_package(plutosvg 0.0.7 CONFIG REQUIRED)

import_shared_library(cpuinfo::cpuinfo cpuinfo.h cpuinfo)

find_package(SoundTouch 2.4.1 CONFIG REQUIRED)

# Arch's SoundTouchTargets.cmake exports bogus interface include paths
# (a stray COMPONENT keyword leaks ".../COMPONENT" and ".../SoundTouch" into
# the list); CMake's generate step aborts on the non-existent directories.
# Keep only the include directories that actually exist.
get_target_property(_soundtouch_includes SoundTouch::SoundTouch INTERFACE_INCLUDE_DIRECTORIES)
set(_soundtouch_includes_clean "")
foreach(_dir IN LISTS _soundtouch_includes)
  if(EXISTS "${_dir}")
    list(APPEND _soundtouch_includes_clean "${_dir}")
  endif()
endforeach()
list(REMOVE_DUPLICATES _soundtouch_includes_clean)
set_target_properties(SoundTouch::SoundTouch PROPERTIES
  INTERFACE_INCLUDE_DIRECTORIES "${_soundtouch_includes_clean}")

# Arch's soundtouch package ships only the C++ SoundTouch library, not the
# SoundTouchDLL C wrapper that DuckStation expects. Expose a header-only shim
# (generated in prepare()) that maps the soundtouch_* C API onto the C++ class.
...
set_target_properties(SoundTouch::SoundTouchDLL PROPERTIES
  ...
  INTERFACE_INCLUDE_DIRECTORIES "${CMAKE_SOURCE_DIR}/dep/soundtouchdll")
