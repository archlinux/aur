# Maintenance helpers for the ruroco AUR package.
#
...
help:
	@echo 'ruroco AUR maintenance targets:'
...
# Full local release prep. Order is enforced via recursive make so it is safe
# under `make -j`. Stops before pushing so you can review the commit.
release:
