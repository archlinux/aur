#!/usr/bin/env bash
APPDIR=/opt/cloudflarespeedtest
...
_CHECK() {
	while true
		do
		if [[ ! -e "${HOME}/nowip_hosts.txt" ]]; then
			...
			if [[ ! -z "${NOWIP}" ]]; then
				echo ${NOWIP} > "${HOME}/nowip_hosts.txt"
				break
			else
				echo "该 IP 不能是空！"
			fi
...
_UPDATE() {
	echo -e "开始测速..."
	NOWIP=$(head -1 "${HOME}/nowip_hosts.txt")

	# 这里可以自己添加、修改 CloudflareST 的运行参数
	./CloudflareST -o "${HOME}/result_hosts.txt"

	# 如果需要 "找不到满足条件的 IP 就一直循环测速下去"，那么可以将下面的两个 exit 0 改为 _UPDATE 即可
	[[ ! -e "${HOME}/result_hosts.txt" ]] && echo "CloudflareST 测速结果 IP 数量为 0，跳过下面步骤..." && exit 0
	...
	BESTIP=$(sed -n "2,1p" "${HOME}/result_hosts.txt" | awk -F, '{print $1}')
	if [[ -z "${BESTIP}" ]]; then
		echo "CloudflareST 测速结果 IP 数量为 0，跳过下面步骤..."
		exit 0
	fi
	echo ${BESTIP} > "${HOME}/nowip_hosts.txt"
	echo -e "\n旧 IP 为 ${NOWIP}\n新 IP 为 ${BESTIP}\n"
