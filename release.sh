...
SHA256=$(sha256sum msk-${VERSION}.tar.gz | awk '{print $1}')
echo SHA256=$SHA256
...
tar xvf msk-${VERSION}.tar.gz
mv msk/arch/install.sh .

#rm -rf msk-${VERSION}      # GitHub adds version to tar's internal folder
rm -rf msk                  # Codeberg doesn't
