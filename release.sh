...
SHA256=$(sha256sum plumero-${VERSION}.tar.gz | awk '{print $1}')
echo SHA256=$SHA256
...
tar xvf plumero-${VERSION}.tar.gz
mv plumero/arch/install.sh .

#rm -rf plumero-${VERSION}      # GitHub adds version to tar's internal folder
rm -rf plumero                  # Codeberg doesn't
