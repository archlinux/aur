...
offline_mirror_path=${work_dir}/${arch}/airootfs/srv/http/arch_offline/os/${arch}

# TODO: Make this info a patch file rather than the full blown build.sh,
#       just to make it easier upon upstream changes to releng template.
...
make_offline_mirror() {
	packages=$(cat ${script_path}/packages.{both,${arch}})
	if  [[ -n ${packages} ]]; then
...
patch_in_local_mirror() {
	sh -c "sed -i 's?\[core\]?\[arch_offline\]\nServer = file://${script_path}/${work_dir}/${arch}/airootfs/srv/http/\$repo/os/\$arch\nSigLevel = Optional TrustAll\n\n&?' ${work_dir}/pacman.conf"
}

install_aur() {
	setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -h -v ^# ${script_path}/packages.aur)" install
}
