...
'--machine[Run an API command on a saved SSH machine]:LABEL-OR-ID:_default' \
...
(machine)
_arguments "${_arguments_options[@]}" : \
":: :_herdr__subcmd__machine_commands" \
"*::: :->machine" \
&& ret=0
    ...
    (machine)
        words=($line[1] "${words[@]}")
        (( CURRENT += 1 ))
        curcontext="${curcontext%:*:*}:herdr-machine-command-$line[1]:"
        case $line[1] in
            (list)
_arguments "${_arguments_options[@]}" : \
'--json[]' \
...
(add)
_arguments "${_arguments_options[@]}" : \
'--label[Set the machine label shown in the sidebar]:LABEL:_default' \
'--remote-session[Set the explicit Herdr session on the remote machine]:NAME:_default' \
':ssh-target:_default' \
&& ret=0
;;
(rename)
...
(start)
_arguments "${_arguments_options[@]}" : \
'--kind[Supported agent kind and canonical executable]:KIND:(pi claude codex gemini cursor devin agy cline omp mastracode opencode copilot kimi kiro droid amp grok hermes kilo qodercli qwen letta maki muse)' \
...
&& ret=0
    ...
    (integration)
        ...
        case $line[1] in
            (install)
_arguments "${_arguments_options[@]}" : \
':target:(pi omp claude codex copilot devin droid kimi opencode kilo hermes qodercli qwen cursor mastracode antigravity-cli grok letta)' \
&& ret=0
;;
(uninstall)
_arguments "${_arguments_options[@]}" : \
...
(( $+functions[_herdr__subcmd__machine_commands] )) ||
_herdr__subcmd__machine_commands() {
    local commands; commands=(
'list:List saved SSH machines' \
'add:Prepare the remote Herdr server and save an SSH machine' \
'rename:Rename a saved SSH machine' \
'remove:Remove a saved SSH machine' \
'enable:Enable a saved SSH machine' \
'disable:Disable a saved SSH machine' \
    )
    _describe -t commands 'herdr machine commands' commands "$@"
}
(( $+functions[_herdr__subcmd__machine__subcmd__add_commands] )) ||
_herdr__subcmd__machine__subcmd__add_commands() {
    local commands; commands=()
    _describe -t commands 'herdr machine add commands' commands "$@"
}
(( $+functions[_herdr__subcmd__machine__subcmd__disable_commands] )) ||
_herdr__subcmd__machine__subcmd__disable_commands() {
    local commands; commands=()
    _describe -t commands 'herdr machine disable commands' commands "$@"
}
(( $+functions[_herdr__subcmd__machine__subcmd__enable_commands] )) ||
_herdr__subcmd__machine__subcmd__enable_commands() {
    local commands; commands=()
    _describe -t commands 'herdr machine enable commands' commands "$@"
}
(( $+functions[_herdr__subcmd__machine__subcmd__list_commands] )) ||
_herdr__subcmd__machine__subcmd__list_commands() {
    local commands; commands=()
    _describe -t commands 'herdr machine list commands' commands "$@"
}
(( $+functions[_herdr__subcmd__machine__subcmd__remove_commands] )) ||
_herdr__subcmd__machine__subcmd__remove_commands() {
    local commands; commands=()
    _describe -t commands 'herdr machine remove commands' commands "$@"
}
(( $+functions[_herdr__subcmd__machine__subcmd__rename_commands] )) ||
_herdr__subcmd__machine__subcmd__rename_commands() {
    local commands; commands=()
    _describe -t commands 'herdr machine rename commands' commands "$@"
}
