...
def is_shell_script(filepath):
    ...
    # ignore errors due to e.g. non-decodable bytes
    except Exception:
        return False
...
def prompt(promptmsg, *msgtypes, **kwargs):
    ...
    if PYVER[0] == 2: response = raw_input(' ').strip()   # noqa F821
...
def isstr(s):
    ...
    and >=3.3.
    """
    try:              return isinstance(s, basestring)  # noqa E741
...
def download_fsl_environment_files(ctx):
    ...
    for envname, build in allenvs:
        ...
        # Add extra packages to (or override existing
        # packages in) main environment. We do no
        # validation on this, so user beware.
        for pkg in ctx.args.extra_package:
            if '=' in pkg: pkg, ver = pkg.split('=', 1)
            else:          pkg, ver = pkg, None
            packages[pkg] = ver
...
def parse_args(argv=None, include=None, parser=None):
    ...
    options = {
        ...
        'extra_package'      : (None, {'action'  : 'append'}),
    ...
    helps = {
        ...
        # Add extra packages/constraints to the
        # main FSL environment
        'extra_package' : argparse.SUPPRESS,
...
def main(argv=None):
    ...
    with tempdir(args.workdir, delete=False) as tmpdir:
        ...
        # Download conda environment files to working directory
