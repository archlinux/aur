...
def fetch_lyrics_bg(artist, title, song_id):
    ...
    # Clean tags for strict matching on Omarchy
    clean_t = title.split(" - ")[0].split(" (")[0].split(" [")[0].split(" feat.")[0].strip()
    clean_a = artist.split(",")[0].split(" & ")[0].strip()
    
    try:
        # THE DEAL: Strict lookup to stop the Oh Land / Nostalgia guessing
        ...
        if r.status_code == 200:
            res = r.json()
            # If plainLyrics is null or it's an instrumental track
            if res.get('plainLyrics') is None or res.get('instrumental'):
                lyric_data["lyrics"] = [{"ts": 0, "txt": "🎶 Instrumental Song 🎶"}]
            else:
...
def get_stats():
    try:
        # Querying NVIDIA GPU specifically for your system
...
def send(text):
    # OSC Address for VRChat Chatbox
    addr, tags = b"/chatbox/input\0\0", b",sT\0"
    ...
    try:
        ...
        if now_ts >= lyric_locked_until:
            if lyric_data["loading"]:
                ...
                if found_new:
                    lyric_locked_until = now_ts + 1.2

        is_paused = (pos == last_pos and stat != "Playing")
        p_flag = " ⏸" if is_paused else ""
        l5 = f"🕒 {fmt_time(pos)} / {fmt_time(length)} [{'▰'*int((pos/length)*10) if length > 0 else ''}{'▱'*(10-int((pos/length)*10)) if length > 0 else ''}]" + p_flag if length > 0 else f"🕒 {fmt_time(pos)} [LIVE]" + p_flag
