...
pipeline {
    ...
    environment {
        VERSION = """${sh(
                returnStdout: true,
                script: "curl https://api.github.com/repos/grafana/pyroscope/releases/latest | jq -r '.tag_name' | cut -c2- | tr -d '\n'"
            )}"""
    }
    stages {
        stage('Checkout') {
        ...
        stage('Get Changelog') {
            steps {
                sh '''
                    curl https://api.github.com/repos/grafana/pyroscope/releases/latest | jq -r '.body' > ChangeLog
                '''
            }
        }
            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: "Arch_AUR", keyFileVariable: 'KEY')]) {
                    sh '''
                        ...
                        git checkout master
                        git merge release
                        git remote add ssh-origin ssh://aur@aur.archlinux.org/pyroscope-bin.git
    ...
    post {
        ...
        success {
            withCredentials([string(credentialsId: 'Discord_Webhook', variable: 'WEBHOOK_URL')]) {
                sh '''
                    curl -H 'Content-Type: application/json' -d '{\"content\": \"✅ Pyroscope v'"$VERSION"' Released to AUR 🎆\"}' ${WEBHOOK_URL}
                '''
            }
        }
        failure {
            withCredentials([string(credentialsId: 'Discord_Webhook', variable: 'WEBHOOK_URL')]) {
                sh '''
                    curl -H 'Content-Type: application/json' -d '{\"content\": \"❌ Pyroscope v'"$VERSION"' Pipeline Failed 🛠️\"}' ${WEBHOOK_URL}
                '''
            }
        }
    }
