...
# Script to update the PKGBUILD to the latest [extra]/linux-hardened
...
def update():
    ...
    kernver, hardenedver = kern_info['pkgver'].rsplit('.', 1)
    ...
    pkgbuild = template.substitute(
        ...
        KERNVER=kernver,
        HARDENEDVER=hardenedver,
        PKGREL=pkgrel,
        KERNNAME=f"{padded_kernver}-{hardenedver}-{pkgrel}-hardened",
        URL=kern_info['url'],
