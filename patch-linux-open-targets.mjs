...
const registryPattern =
  /var ([A-Za-z_$][\w$]*)=\[([^\]]+)\],([A-Za-z_$][\w$]*)=t\.(Pr|Kr)\(`open-in-targets`\);/;
...
const [registrySource, registryName, registryEntries, loggerName, loggerFactoryName] =
  registryMatch;
...
const linuxPatch = [
  "function linuxResolveAbsoluteCommand(e){let t=Jp(e);return t&&(0,o.existsSync)(t)?t:null}",
  ...
  "function linuxResolveDesktopExec(e){let t=e.trim().match(/^\"([^\"]+)\"|^\\x27([^\\x27]+)\\x27|^(\\S+)/),n=t?.[1]??t?.[2]??t?.[3];if(!n)return null;return linuxResolveAbsoluteCommand(n)??(()=>{let e=Fp(n);return e?Jp(e):null})()}",
  ...
  "function linuxResolveEditorTarget(e,t=[],n=[]){for(let t of e){let e=Fp(t);if(e){let t=Jp(e);if(t)return t}}for(let e of t){let t=linuxResolveAbsoluteCommand(e);if(t)return t}let r=n.length>0?linuxFindDesktopEntryExec(n):null;return r??(n.length>0?linuxFindAppImage(n):null)}",
  "function linuxFileManagerDetect(){return Fp(`xdg-open`)??linuxResolveAbsoluteCommand(`/usr/bin/xdg-open`)}",
  "function linuxOpenFileManagerPath(e){let t=e;for(;;){if((0,o.existsSync)(t))break;let e=(0,i.dirname)(t);if(e===t){t=null;break}t=e}let n=t??e;if((0,o.existsSync)(n)&&(0,o.statSync)(n).isFile())n=(0,i.dirname)(n);return Pw(n)}",
  "var linuxVscode={id:`vscode`,platforms:{linux:{label:`VS Code`,icon:`apps/vscode.png`,kind:`editor`,detect:()=>linuxResolveEditorTarget([`code`,`code-oss`],[`/usr/bin/code`,`/usr/bin/code-oss`,`/snap/bin/code`],[`visual studio code`,`code`,`code-oss`]),args:vC,supportsSsh:!0}}},",
  "linuxVscodeInsiders={id:`vscodeInsiders`,platforms:{linux:{label:`VS Code Insiders`,icon:`apps/vscode-insiders.png`,kind:`editor`,detect:()=>linuxResolveEditorTarget([`code-insiders`],[`/usr/bin/code-insiders`,`/snap/bin/code-insiders`],[`insiders`,`code-insiders`]),args:vC,supportsSsh:!0}}},",
  "linuxCursor={id:`cursor`,platforms:{linux:{label:`Cursor`,icon:`apps/cursor.png`,kind:`editor`,detect:()=>linuxResolveEditorTarget([`cursor`],[`/usr/bin/cursor`,`/opt/Cursor/cursor`,`/opt/cursor/cursor`],[`cursor`]),args:vC,supportsSsh:!0}}},",
  "linuxWindsurf={id:`windsurf`,platforms:{linux:{label:`Windsurf`,icon:`apps/windsurf.png`,kind:`editor`,detect:()=>linuxResolveEditorTarget([`windsurf`],[`/usr/bin/windsurf`,`/opt/Windsurf/windsurf`,`/opt/windsurf/windsurf`],[`windsurf`]),args:vC,supportsSsh:!0}}},",
  "linuxZed={id:`zed`,platforms:{linux:{label:`Zed`,icon:`apps/zed.png`,kind:`editor`,detect:()=>linuxResolveEditorTarget([`zed`],[`/usr/bin/zed`,`/opt/zed/zed`,`/opt/Zed/zed`],[`zed`]),args:Ow}}},",
...
const replacement = `${linuxPatch}var ${registryName}=[${linuxTargets},${registryEntries}],${loggerName}=t.${loggerFactoryName}(\`open-in-targets\`);`;
