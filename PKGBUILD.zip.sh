...
case "${_autoupdate::1}" in
  't'|'y'|'1')
    _response=$(curl "https://api.github.com/repos/trixon/mapton/releases" -s)
...
prepare() {
  # exec script
  sed -Ei \
    -e 's@(\s+DEFAULT_USERDIR_ROOT)=\$\{HOME\}/\.\$\{APPNAME\}$@\1="$XDG_CONFIG_HOME/mapton"@' \
    -e 's@(\s+DEFAULT_CACHEDIR_ROOT)=\$\{HOME\}/\.cache/\$\{APPNAME\}$@\1="$XDG_CACHE_HOME/mapton"@' \
