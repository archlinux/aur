...
sed -i "s/^pkgver=.*/pkgver=${pkgver}/;s/^pkgrel=.*/pkgrel=${pkgrel}/;s/^_upver=.*/_upver=${_upver}/" PKGBUILD

# Update source and hash sums
# Since the source only contains 1 file, we can directly get its hash
new_hash=$(get_file "${url}/releases/latest/download/${_pkg}-${_upver}.zip" | sha256sum | cut -f1 -d' ')
