...
LATEST_VERSION=$(wget --quiet -O - https://desktop.download.sipgate.com/latest.yml | yq -r '.version')
LATEST_VERSION_LINUX=$(wget --quiet -O - https://desktop.download.sipgate.com/latest-linux.yml | yq -r '.version')

echo "installed version: ${INSTALLED_VERSION}"
echo "latest version: ${LATEST_VERSION}"
echo "latest linux version: ${LATEST_VERSION_LINUX}"
echo

if [ "x$LATEST_VERSION" != "x$INSTALLED_VERSION" -a "x$LATEST_VERSION_LINUX" != "x$INSTALLED_VERSION" ]; then
