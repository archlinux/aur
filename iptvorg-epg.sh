...
# Revision Date: 29/06/2026

# Variables
EPG_USR=$(whoami)
EPG_EXE=$(basename "$0")
EPG_USR_HOME=$(getent passwd "$EPG_USR" | cut -d: -f6)
EPG_SOURCE="/usr/share/iptvorg-epg-git"
...
checkDir() {
    ...
    if [[ ! -d "$EPG_CFGDIR" ]]; then
        ...
        # Symlink the project files so npm can find them
        ln -sf "$EPG_SOURCE/package.json" "$EPG_CFGDIR/package.json"
        ln -sf "$EPG_SOURCE/package-lock.json" "$EPG_CFGDIR/package-lock.json"
        ln -sf "$EPG_SOURCE/node_modules" "$EPG_CFGDIR/node_modules"
        ln -sf "$EPG_SOURCE/scripts" "$EPG_CFGDIR/scripts"
        ln -sf "$EPG_SOURCE/sites" "$EPG_CFGDIR/sites"
        ln -sf "$EPG_SOURCE/tsconfig.json" "$EPG_CFGDIR/tsconfig.json"
        ln -sf "$EPG_SOURCE/temp" "$EPG_CFGDIR/temp"

        # Copy the sample channels file as a starting point
        if [[ -f "$EPG_SOURCE/my.channels.xml" ]]; then
...
updateEpg() {
    ...
    # Prepare parameters
    local mychannel="" mysite="" mydays="" myoutput=""
    [[ "$channels_on" -eq 1 ]] && mychannel="--channels=${EPG_CCH}"
    [[ "$site_on" -eq 1 ]] && mysite="--sites=${EPG_SITE}"
    [[ "$days_on" -eq 1 ]] && mydays="--days=${EPG_DAYS}"
    myoutput="-o tmp_all.xml"
    ...
    # Run the command
    cd "$EPG_CFGDIR" || exit
    printf "Running: npm run grab -- %s %s %s %s\n" "$mysite" "$mychannel" "$mydays" "$myoutput"
    npm run grab -- $mysite $mychannel $mydays $myoutput 2>&1
...
helpMsg() {
    cat <<-HELPEOF
        ...
        Options:
          ...
          -s, --site <name>     Specify the site to parse (comma-separated for multiple)
