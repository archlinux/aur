...
license=$(yq eval -o=json "$pytoml" | jq -r '.project.license.text')
url=$(yq eval -o=json "$pytoml" | jq -r '.project.urls."Source Code"')
depends=$(yq eval -o=json "$pytoml" | jq -r '.project.dependencies[]' | tr 'A-Z' 'a-z' | grep -oP '^[a-z0-9_\-]+' | sed 's|^|python-|' | sed 's|python-python-|python-|' | sed '/^python-python$/d' | sort -u | tr '\n' ' ' | sed 's| $||')
