...
pkg_detect_latest() {
	local api_url="https://api.github.com/repos/xifan2333/dmnotifier/releases/latest"
...
pkg_get_update_params() {
	local version="$1"
	local base_url="https://github.com/xifan2333/dmnotifier/releases/download/v${version}"
	local filename_x86_64="dmnotifier-linux-amd64"
...
pkg_update_files() {
	...
	local base_url="${url%/*}"
	local url_x86_64="${base_url}/dmnotifier-linux-amd64"
	local url_aarch64="${base_url}/dmnotifier-linux-arm64"

	sed -i "s/^pkgver=.*/pkgver=${pkgver}/" "${pkgbuild}"
	sed -i "s/^pkgrel=.*/pkgrel=1/" "${pkgbuild}"
	sed -i "s|^source_x86_64=.*|source_x86_64=(\"dmnotifier-linux-amd64-\${pkgver}::${url_x86_64}\")|" "${pkgbuild}"
	sed -i "s|^source_aarch64=.*|source_aarch64=(\"dmnotifier-linux-arm64-\${pkgver}::${url_aarch64}\")|" "${pkgbuild}"
