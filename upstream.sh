...
pkg_detect_latest() {
	local api_url="https://api.github.com/repos/xifan2333/xrwm/releases/latest"
...
pkg_get_update_params() {
	local version="$1"
	local base_url="https://github.com/xifan2333/xrwm/releases/download/v${version}"
	local filename_x86_64="xrwm-${version}-x86_64-unknown-linux-gnu.tar.gz"
...
pkg_update_files() {
	...
	local base_url="${url%/*}"
	local url_x86_64="${base_url}/xrwm-${pkgver}-x86_64-unknown-linux-gnu.tar.gz"

	sed -i "s/^pkgver=.*/pkgver=${pkgver}/" "${pkgbuild}"
	sed -i "s/^pkgrel=.*/pkgrel=1/" "${pkgbuild}"
	sed -i "s|^source_x86_64=.*|source_x86_64=(\"\${_pkgname}-\${pkgver}-x86_64.tar.gz::${url_x86_64}\")|" "${pkgbuild}"
