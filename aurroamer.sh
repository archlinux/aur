...
cachedaur="$HOME/.cache/pkgbrowser-aur.txt"
...
if ! pacman -Slq > "$repopackages"; then
    echo "failed to fetch official packages"
    exit 1
fi
...
case "$mode" in
    1)
        while IFS= read -r pkg; do printf "[official]   %s\n" "$pkg"; done < "$repopackages" > "$total"
        ;;
    2)
        while IFS= read -r pkg; do printf "[aur]        %s\n" "$pkg"; done < "$aurpackages" > "$total"
        ;;
    3)
        while IFS= read -r pkg; do printf "[aur]        %s\n" "$pkg"; done < "$aurpackages" > "$total"
        while IFS= read -r pkg; do printf "[official]   %s\n" "$pkg"; done < "$repopackages" >> "$total"
...
echo
read -rp "browse selected packages with fzf? [y/N] " confirm
[[ ! "$confirm" =~ ^[Yy]$ ]] && exit 0
...
while true; do
    ...
    pkgtype=$(awk '{print $1}' <<< "$selection")
    pkgname=$(awk '{print $2}' <<< "$selection")
    clear
    echo "you selected: $pkgname $pkgtype"
    echo
    ...
    while true; do
        ...
        case "$choice" in
            1)
                ...
                echo
                read -rp "press any key to return"
                clear
                ;;
            2)
                clear
