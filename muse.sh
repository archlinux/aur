...
if [[ "$(uname -m)" == "x86_64" ]] && ! grep -q -m1 "avx2" /proc/cpuinfo 2>/dev/null; then
  if command -v intel-sde >/dev/null 2>&1; then
    exec intel-sde -hsw -no-follow-child -p -xyzzy -p -virtual_segments -- "$REAL_BIN" "$@"
  elif command -v sde64 >/dev/null 2>&1; then
    exec sde64 -hsw -no-follow-child -p -xyzzy -p -virtual_segments -- "$REAL_BIN" "$@"
  elif command -v sde >/dev/null 2>&1; then
    exec sde -hsw -no-follow-child -p -xyzzy -p -virtual_segments -- "$REAL_BIN" "$@"
  else
