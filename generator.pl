...
sub _get_data {
    ...
    my $package_x86_64 = eval { _get_part($ua, $uri, qr($locale.linux-x86_64.tar.bz2$)); };
    die "Can't get x86_64 package: $@" if $@;
    ...
    my $package_i686 = eval { _get_part($ua, $uri, qr($locale.linux-i686.tar.bz2$)); };
    die "Can't get i686 package: $@" if $@;
