...
local_typora_ver=$(echo $local_ver | cut -d "_" -f 1)
local_typora_sum=$(cat .SRCINFO | rg sha512sums | head -1 | cut -d " " -f 3)
local_plugin_ver=$(echo $local_ver | cut -d "_" -f 3)
local_plugin_sum=$(cat .SRCINFO | rg sha512sums | tail -1 | cut -d " " -f 3)

echo "Fetching the latest version..."
typora_url=$(curl -s https://typoraio.cn/ | rg "https://download2.typoraio.cn/linux/typora_" | cut -d "|" -f 2 | cut -d "\"" -f 28)
latest_typora_ver=$(echo $typora_url | cut  -d "_" -f 2)
latest_typora_file="typora_${latest_typora_ver}_amd64.deb"
...
latest_plugin_file="${latest_plugin_ver}.tar.gz"

latest_ver=${latest_typora_ver}_plugin_${latest_plugin_ver}
types=("Typora" "Typora-plugin")
local_vers=($local_typora_ver $local_plugin_ver)
latest_vers=($latest_typora_ver $latest_plugin_ver)
local_sums=($local_typora_sum $local_plugin_sum)
latest_files=($latest_typora_file $latest_plugin_file)
urls=($typora_url $plugin_url)
