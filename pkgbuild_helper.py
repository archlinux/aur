...
BIN_URL = "https://dldir1v6.qq.com/weixin/Universal/Linux/WeChatLinux_x86_64.deb"

def fetch_latest_version():
    """下载 .deb 文件并提取版本号"""
    deb_file = "WeChatLinux_x86_64.deb"
    try:
        # 下载 .deb 文件
        urllib.request.urlretrieve(BIN_URL, deb_file)

        # 提取 control 文件并解析版本号
        control_content = subprocess.check_output(
            f"ar p {deb_file} control.tar.xz | tar -xJOf - ./control", 
            shell=True, text=True
        )
        match = re.search(r"^Version:\s*([\d.]+)", control_content, re.MULTILINE)
