...
#
# Entry point: the bundled CLI launcher at /opt/Paseo/resources/bin/paseo runs
# the app binary as plain Node (ELECTRON_RUN_AS_NODE=1) via the node-entrypoint
# runner, so it spawns the daemon supervisor correctly. Do NOT use /usr/bin/paseo
# here — that is the Electron GUI wrapper and cannot start the node-mode
# supervisor.
#
