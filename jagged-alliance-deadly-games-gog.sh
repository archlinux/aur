...
configs=('-conf' 'dosboxjadg.conf')

if [ "${START_SERVER:=0}" -eq 1 ] && [ ! "$START_SETTINGS" ]; then
    configs+=('-conf' 'dosboxjadg_server.conf')
    ENABLE_IPX=1
fi

if [ "${START_CLIENT:=0}" -eq 1 ] && [ "${START_SERVER:=0}" -ne 1 ] \
        && [ ! "$START_SETTINGS" ]; then
    configs+=('-conf' 'dosboxjadg_client.conf')
    ENABLE_IPX=1
fi
...
if [ "$START_SETTINGS" ]; then
    configs+=('-conf' 'dosboxjadg_settings.conf')
else
    configs+=('-conf' 'dosboxjadg_single.conf')
fi
