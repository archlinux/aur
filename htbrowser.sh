...
#echo $RUID
if [ "${RUID}" = "0" ];then
    exec -a "$0" "${_RUNNAME}" --no-sandbox --disable-print-preview "${CHROMIUM_DISTRO_FLAGS}" "${CHROMIUM_FLASH_FLAGS}" "$@" || exit $?
else
    exec -a "$0" "${_RUNNAME}" --disable-gpu "${CHROMIUM_DISTRO_FLAGS}" "${CHROMIUM_FLASH_FLAGS}" "$@" || exit $?
fi